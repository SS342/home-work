...
def select():
    ...
    if tmp:
        text1w.configure(text = 'âõîä âûïîëíåí')
    else:
        text1w.configure(text = 'âõîä íå âûïîëíåí')
...
notebook.add(f1, text = 'äîáàâèòü')
notebook.add(f2, text = 'ðåêîìåíäàöèè')
notebook.pack()
...
text1qqqq = ttk.Label(f1,text="ïî âñåì âîïðîñàì îáðàùàéòåñü â îáîðîòíÿ ;)", width = 150) 
text1qqqq.pack(side=tk.TOP)


but1 = ttk.Button(f1, text="âîéòè",command = select)                                  #thiiiiis
but1.pack(side=tk.TOP)

but1q = ttk.Button(f1, text="ðåãèñòðàöèÿ",command = registr)
...
lb.insert(tk.END, '1-àäìèí','2-êîíñóëüòàíò','3-ïðîãðàììèñò',"4-âåá-äèçàéíåð","5-ôîòîãðàô","6-äèðåêòîð","7-çàìåñòèòåëü","8-ñåêðåòàðü")
lb.grid(row = 1,column = 0)

text2 = ttk.Label(f4,text="<--ñïèñîê äîñòóïíûõ äîëæíîñòåé:", width = 30) 
text2.grid(row = 1, column = 1)

text3 = ttk.Label(f4,text="èìÿ:", width = 10) 
text3.grid(row = 2, column = 0)

entry2 = ttk.Entry(f4, width = 22)
entry2.grid(row = 3, column = 1)

text4 = ttk.Label(f4,text="ôàìèëèÿ:", width = 10) 
text4.grid(row = 4, column = 0)

entry3 = ttk.Entry(f4, width = 22)
entry3.grid(row = 5, column = 1)

text5 = ttk.Label(f4,text="îò÷åñòâî:", width = 10) 
text5.grid(row = 6, column = 0)

entry4 = ttk.Entry(f4, width = 22)
entry4.grid(row = 7, column = 1)

text6 = ttk.Label(f4,text="íîìåð âàøåé äîëæíîñòè:", width = 25) 
...
lb1.insert(tk.END, '1-ôóòáîëêè','2-îáóâü','3-íîóòáóêè',"4-ñëàäîñòè","5-êàðòèíû")
lb1.grid(row = 1,column = 0)

text7 = ttk.Label(f5,text="<--ñïèñîê äîñòóïíûõ òèïîâ:", width = 30) 
text7.grid(row = 1, column = 1)

text8 = ttk.Label(f5,text="ñïèñîê äîñòóïíûõ áðåíäîâ-->", width = 30) 
...
var = tk.BooleanVar()
cb = Checkbutton(f6,text = 'îòêàçàòüñÿ îò ðàññûëêè', variable = var, command = qw)
cb.pack(side=tk.TOP)
